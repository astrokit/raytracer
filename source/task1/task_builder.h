...
struct TaskBuilder
{
	...
	virtual void defineCube(const float3& position, const float3& rotation, const float3& scale, const std::string& material) = 0;
	...
	virtual void defineSpotLight(const float3& p, const float3& color, const float3& dir, const float& angle) = 0;
