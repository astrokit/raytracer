...
struct Cube {
  ...
  float4x4 inverse_rotation_matrix;
...
class Scene {
  ...
  const Cone* findClosestHitCone(const float3& p, const float3& d,
                                 float& t) const;
  const Cube* findClosestHitCube(const float3& p, const float3& d,
                                 float& t) const;
  bool intersectsCone(const float3& p, const float3& d, float t_min,
                      float t_max) const;
  bool intersectsCube(const float3& p, const float3& d, float t_min,
                      float t_max) const;
  ...
  HitPoint hit(const Cone& cone, const float3& p, const float3& d,
               float t) const;
  HitPoint hit(const Cube& cube, const float3& p, const float3& d,
               float t) const;
 ...
 public:
  Scene(std::unique_ptr<float3[]>&& positions,
        ...
        std::unique_ptr<Cube[]>&& cubes, std::size_t num_cubes,
