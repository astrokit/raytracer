...
bool BVHLeafNode::intersectsRay(const float3& p, const float3& d,
	const triangle_t* triangles, std::size_t num_triangles,
	const float3* vertices, float t_min, float t_max) const {
...
const triangle_t* BVHLeafNode::findClosestHit(const float3& p, const float3& d,
	const triangle_t* triangles,
	std::size_t num_triangles,
	const float3* vertices, float& t,
	float& lambda_1, float& lambda_2) const {

	const triangle_t* hit = nullptr;
