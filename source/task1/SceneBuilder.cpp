...
void SceneBuilder::addCube(const float3& position, const float3& rotation, const float3& scale, const std::string& material)
{
	float4x4 translation_matrix = getTranslationMatrix(position);
	float4x4 rotation_matrix = getRotationMatrix(rotation);
	float4x4 model_matrix = getModelMatrix(translation_matrix, rotation_matrix);
	*extendBuffer(cubes, num_cubes++, 1) = { position, rotation, scale, lookupMaterialID(material), 
		translation_matrix, rotation_matrix, model_matrix,
		math::inverse(rotation_matrix), math::inverse(model_matrix)};
}
...
Scene SceneBuilder::finish()
{
	...
	return { std::move(positions), std::move(normals), std::move(texcoords), num_vertices, /*std::move(triangles), std::move(material_ids), num_triangles,*/ std::move(bvh), std::move(planes), num_planes, std::move(cones), num_cones, std::move(cubes), num_cubes, std::move(materials), num_materials, std::move(textures), num_textures };
}
