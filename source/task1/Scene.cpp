...
Scene::Scene(std::unique_ptr<float3[]>&& positions, std::unique_ptr<float3[]>&& normals, std::unique_ptr<float2[]>&& texcoords, std::size_t num_vertices, /*std::unique_ptr<triangle_t[]>&& triangles, std::unique_ptr<int[]>&& material_ids, std::size_t num_triangles,*/ BVHRoot&& bvh, std::unique_ptr<Plane[]> planes, std::size_t num_planes, std::unique_ptr<Cone[]>&& cones, std::size_t num_cones, std::unique_ptr<Cube[]>&& cubes, std::size_t num_cubes, std::unique_ptr<Material[]>&& materials, std::size_t num_materials, std::unique_ptr<image2D<std::uint32_t>[]>&& textures, std::size_t num_textures)
...
void Scene::deserialize(std::istream& file)
{
  ...
  num_cubes = read<std::uint32_t>(file);
  if (num_cubes)
    cubes = readBunch<Cube>(file, num_cubes);
...
std::ostream& Scene::serialize(std::ostream& file) const
{
  ...
  if (num_cubes > std::numeric_limits<std::uint32_t>::max())
    throw std::runtime_error("too many cubes");
  write(file, static_cast<std::uint32_t>(num_cubes));
  write(file, cubes.get(), num_cubes);
...
HitPoint Scene::hit(const Cube& cube, const float3& p, const float3& d, float t) const
{
  float4 p_local = cube.inverse_model_matrix * float4(p, 1.0f);
  float4 d_local = cube.inverse_rotation_matrix * float4(d, 1.0f);

  float3 position = float3(p_local.x, p_local.y, p_local.z) + t * float3(d_local.x, d_local.y, d_local.z);
  float min_d = std::numeric_limits<float>::max();
  float3 normal = float3(0.1f, 0.1f, 0.1f);

  // Min and Max of local AABB
  float3 p_min = -float3(cube.scale * 0.5f);
  float3 p_max = float3(cube.scale * 0.5f);

  float dist = 0.0f;

  if ((dist = std::abs(p_min.x - position.x)) < min_d)
  {
    min_d = dist;
    normal = float3(-1.0f, 0.0f, 0.0f);
  }

  if ((dist = std::abs(p_min.y - position.y)) < min_d)
  {
    min_d = dist;
    normal = float3(0.0f, -1.0f, 0.0f);
  }

  if ((dist = std::abs(p_min.z - position.z)) < min_d)
  {
    min_d = dist;
    normal = float3(0.0f, 0.0f, -1.0f);
  }

  if ((dist = std::abs(p_max.x - position.x)) < min_d)
  {
    min_d = dist;
    normal = float3(1.0f, 0.0f, 0.0f);
  }

  if ((dist = std::abs(p_max.y - position.y)) < min_d)
  {
    min_d = dist;
    normal = float3(0.0f, 1.0f, 0.0f);
  }

  if ((dist = std::abs(p_max.z - position.z)) < min_d)
  {
    min_d = dist;
    normal = float3(0.0f, 0.0f, 1.0f);
  }

  auto& mat = materials[cube.material];

  float4 n_world = cube.rotation_matrix * float4(normal, 1.0f);

  return { p + t * d, float3(n_world.x, n_world.y, n_world.z), mat.diffuse, mat.specular, mat.shininess };
}
...
const Cube* Scene::findClosestHitCube(const float3& p, const float3& d, float& t) const
{
  return findClosestHitCubes(p, d, cubes.get(), num_cubes, t);
}

bool Scene::intersectsCube(const float3& p, const float3& d, float t_min, float t_max) const
{
  return intersectsRayCube(p, d, cubes.get(), num_cubes, t_min, t_max);
}
...
std::optional<HitPoint> Scene::findClosestHit(const float3& p, const float3& d) const
{
  ...
  float t_cube = std::numeric_limits<float>::infinity();
  float t_min = std::numeric_limits<float>::infinity();
  const Cone* cone = nullptr;
  const Plane* plane = nullptr;
  const Cube* cube = nullptr;

  plane = findClosestHitPlane(p, d, t_plane);

  if (t_min > t_plane && plane)
  {
    t_min = t_plane;
  }

  cone = findClosestHitCone(p, d, t_cone);

  if (t_min > t_cone && cone)
  {
    t_min = t_cone;
    plane = nullptr;
  }
  ...
  cube = findClosestHitCube(p, d, t_cube);

  if (t_min > t_cube && cube)
  {
    t_min = t_cube;
    plane = nullptr;
    cone = nullptr;
...
bool Scene::intersectsRay(const float3& p, const float3& d, float t_min, float t_max) const
{
  ...
  return bvh->intersectsRay(p, d, positions.get(), t_min, t_max) || intersectsPlane(p, d, t_min, t_max) || intersectsCone(p, d, t_min, t_max) || intersectsCube(p, d, t_min, t_max);
}
