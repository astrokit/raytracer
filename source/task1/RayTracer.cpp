...
image2D<float3> RayTracer::render(int res_x, int res_y, const Scene& scene, const Camera& camera, const Pointlight* lights, std::size_t num_lights, const Spotlight* spotlights, std::size_t num_spotlights, const float3& background_color, int max_bounces, int num_threads)
{
	image2D<float3> framebuffer(res_x, res_y);

	std::cout << "rendering..." << std::endl;

	launchTiled([&framebuffer, &scene, &camera, lights, num_lights, spotlights, num_spotlights, &background_color, max_bounces](int left, int top, int right, int bottom, int n, int N, int thread_id)
	{
		::render(framebuffer, left, top, right, bottom, scene, camera, lights, num_lights, spotlights, num_spotlights, background_color, max_bounces);
...
image2D<RGB32F> RayTracer::renderShadingTest(int res_x, int res_y, const Scene& scene, const Camera& camera, const Pointlight* lights, std::size_t num_lights, const Spotlight* spotlights, std::size_t num_spotlights, int num_threads) const
{
	image2D<RGB32F> img(res_x, res_y);

	render360(img, [&scene, lights, num_lights, spotlights, num_spotlights](const float3& p, const float3& d) -> RGB32F
	{
		if (auto hit = scene.findClosestHit(p, d))
		{
			auto c = shade(p, d, *hit, scene, lights, num_lights, spotlights, num_spotlights);
			return { c.x, c.y, c.z };
		}
