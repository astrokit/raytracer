...
void Task::writeCachedTask(const std::filesystem::path& filename) const
{
	...
	write(file, static_cast<std::uint16_t>(size(spotlights)));
	for(const auto& l : spotlights){
        write(file, l.position.x); write(file, l.position.y); write(file, l.position.z);
        write(file, l.color.x); write(file, l.color.y); write(file, l.color.z);
        write(file, l.direction.x); write(file, l.direction.y); write(file, l.direction.z);
        write(file, l.angle);
	}
}
...
bool Task::readCachedTask(const std::filesystem::path& scene_cache_file_name)
{
    ...
    auto num_spotlights = read<std::uint16_t>(file);
    spotlights.reserve(num_spotlights);
    for (unsigned int i = 0; i < num_spotlights; ++i)
        spotlights.push_back({ { read<float>(file), read<float>(file), read<float>(file) }, { read<float>(file), read<float>(file), read<float>(file) },
                               { read<float>(file), read<float>(file), read<float>(file) }, read<float>(file)});
...
void Task::defineCube(const float3& position, const float3& rotation, const float3& scale, const std::string& material)
{
	std::get<SceneBuilder>(scene).addCube(position, rotation, scale, material);
}
...
void Task::defineSpotLight(const float3& p, const float3& color, const float3& dir, const float& angle){
    spotlights.push_back({ p, color, dir, angle });
}

void Task::render(const std::filesystem::path& output_file, int res_x, int res_y, int max_bounces, int num_threads)
{
	auto start = std::chrono::steady_clock::now();
	auto framebuffer = raytracer.render(res_x > 0 ? res_x : canvas_width, res_y > 0 ? res_y : canvas_height, std::get<Scene>(scene), camera, data(lights), size(lights),
                                     data(spotlights), size(spotlights), background_color, max_bounces, num_threads);
...
{
	auto img = raytracer.renderShadingTest(res_x > 0 ? res_x : 1024, res_y > 0 ? res_y : 512, std::get<Scene>(scene), camera, data(lights), size(lights), data(spotlights), size(spotlights), num_threads);
	PFM::saveRGB32F(output_file.u8string().c_str(), img);
}
