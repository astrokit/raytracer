...
namespace
{
    ...
    class SpotLightObjectConsumer : public config::RejectingParser
    {
        float3 p;
        float3 color;
        float3 dir;
    ...
    public:
        SpotLightObjectConsumer(config::Parser* ret, TaskBuilder& builder)
                : ret(ret), builder(builder)
        {
        }
        ...
        Parser* parseKey(std::string&& str) override
        {
            if (str == "p")
                ...
                return &consumers.emplace<config::VectorParser<float, 3>>(this, dir);
            else if (str == "angle")
                return &consumers.emplace<config::NumberConsumer<float>>(this, angle);
        ...
        Parser* endObject(std::size_t num_members) override
        {
            builder.defineSpotLight(p, color, normalize(dir), angle);
            return ret;
        }
    };

    class SpotLightsConsumer : public config::RejectingParser
    {
        config::Parser* ret;
        TaskBuilder& builder;
        ...
        std::variant<
                std::monostate,
                SpotLightObjectConsumer
        > consumers;

    public:
        SpotLightsConsumer(config::Parser* ret, TaskBuilder& builder)
                : ret(ret), builder(builder)
        {
        }
        ...
        Parser* beginObject() override
        {
            return &consumers.emplace<SpotLightObjectConsumer>(this, builder);
        }
    ...
    using SpotLightArrayParser = config::ArrayParser<SpotLightsConsumer>;
	...
	class CubeSceneObjectConsumer : public config::RejectingParser
	{
		float3 position = { NaN, NaN, NaN };
		float3 rotation = { NaN, NaN, NaN };
		float3 scale = { NaN, NaN, NaN };
		std::string material;
	...
	public:
		CubeSceneObjectConsumer(config::Parser* ret, TaskBuilder& builder)
			: ret(ret), builder(builder)
		{
		}
		...
		{
			if (str == "position")
				return &consumers.emplace<config::VectorParser<float, 3>>(this, position);
			else if (str == "rotation")
				return &consumers.emplace<config::VectorParser<float, 3>>(this, rotation);
			else if (str == "scale")
				return &consumers.emplace<config::VectorParser<float, 3>>(this, scale);
			else if (str == "material")
				return &consumers.emplace<config::StringConsumer>(this, material);
		...
		Parser* endObject(std::size_t num_members) override
		{
			if (std::isnan(position.x) || std::isnan(position.y) || std::isnan(position.z))
				throw config::parse_error("cube missing position");

			if (std::isnan(rotation.x) || std::isnan(rotation.y) || std::isnan(rotation.z))
				throw config::parse_error("cube missing rotation");

			if (std::isnan(scale.x) || std::isnan(scale.y) || std::isnan(scale.z))
				throw config::parse_error("cube missing scale");

			builder.defineCube(position, rotation, scale, material);
			return ret;
		}
	};
	...
	public:
		...
		Parser* parseString(std::string&& str) override
		{
			if (str == "obj")
				...
				return &consumers.emplace<CubeSceneObjectConsumer>(ret, builder);
	...
	public:
      ...
      else if (str == "spotlights")
          return &consumers.emplace<SpotLightArrayParser>(this, builder);
